...
fn main() {
    ...
    let upper_left = parse_complex(&args[3])
        .expect("error parsing upper left complex values");
    let lower_right = parse_complex(&args[4])
        .expect("error parsing lower right complex values");
...
fn write_image(filename: &str, pixels: &Vec<u8>, bounds: (usize, usize)) -> Result<(), std::io::Error> {
...
/// The `upper_left` and `lower_right` parameters are points
